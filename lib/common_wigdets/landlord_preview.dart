...
class PdfLandlordViewScreen extends StatefulWidget {
  final String? url;
  ...
  @override
  State<PdfLandlordViewScreen> createState() => _PdfLandlordViewScreenState();
}

class _PdfLandlordViewScreenState extends State<PdfLandlordViewScreen> {
  ...
  Future<void> _downloadCurrentPdf() async {
    try {
      ...
      /// Case 1: If edited PDF exists → use _pdfBytes
      if (_pdfBytes != null) {
        await file.writeAsBytes(_pdfBytes!);
      }
      /// Case 2: If no bytes but URL exists → download
      else if (widget.url != null && widget.url!.isNotEmpty) {
        await dio_lib.Dio().download(widget.url!, filePath);
      } else {
        EasyLoading.dismiss();
          ...
          TextButton(
            onPressed: () async {
              ...
              if (filePath != null) {
                ...
                Get.snackbar("Error", "Failed to generate signed PDF");
              }
            },
            child: const Text(
              "SAVE",
